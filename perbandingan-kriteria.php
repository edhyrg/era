...
if (!empty($_POST)) {
    ...
    foreach (data_kriteria() as $x) {
        ...
        foreach (data_kriteria() as $y) {
            ...
            echo "<td class=\"text-center\">" . round(bobot_kriteria($x[0], $y[0])['nilai'], 4) . "</td>";
            $yy++;
        }
        echo '</tr>';
        $xx++;
    ...
    for ($x = 0; $x < $matrix->getSize()[1]; $x++) { //loop sebanyak jml data di baris (banyaknya kolom)
        echo '<th class="text-center">' . round(array_sum($matrix->getCol($x)), 4) . '</th>';
    }
    echo '</table>';
    ...
    foreach (data_kriteria() as $x) {
        ...
        foreach (data_kriteria() as $y) {
            ...
            // echo "<td class=\"text-center\">" . round((bobot_kriteria($x[0], $y[0])['nilai'] / array_sum($matrix->getCol($yy))), 4) . "</td>";
            $yy++;
        }
        echo '</tr>';
        $xx++;
    ...
    $data_matrix = array();
    for ($x = 0; $x < count($kriteria); $x++) {
        $b = round(array_sum($matrix_norm->getRow($x)) / count($matrix_norm->getRow($x)), 4);
        $data_matrix[$x][0] = $b;
    ...
    $jml = 0;
    for ($x = 0; $x < count($kriteria); $x++) {
        $t = round($mul_mat->getData()[$x][0] / $matrix_eign->getData()[$x][0], 4);
        $jml += round($t, 4);
        echo "<tr><td>{$kriteria[$x][1]}</td><td>{$mul_mat->getData()[$x][0]}</td><td>$t</td></tr>";
    }
    echo '</table>';
    $t = round(1 / count($kriteria) * $jml, 4);
    echo 't = 1/' . count($kriteria) . '*' . $jml . ' = ' . $t . '<br>';
    $jumlahk = count($kriteria) - 1;
    $ci = round(($t - count($kriteria)) / $jumlahk, 4);
    echo "CI = ($t-" . count($kriteria)  . ')/' . $jumlahk . ' = ' . $ci . '<br>';
    $ri = array(
        1 => 0,
        2 => 0,
        3 => 0.58,
    ...
    $cr = round($ci / $ri[count($kriteria)], 4);
    if ($cr <= 0.10) {
