...
if (!empty($_POST)) {
    ...
    if (empty($pesan_error)) {
        $q = $conn->prepare("UPDATE kriteria SET nama='$nama', atribut='$atribut' WHERE id_kriteria='$id'");
        $q->execute();
        ob_clean();
    ...
    @$id = $_GET['id'];
    $q = $conn->prepare("SELECT * FROM kriteria JOIN atribut ON kriteria.atribut=atribut.id_atribut WHERE kriteria.id_kriteria='$id'");
    $q->execute();
    @$data = $q->fetchAll()[0];
    if ($data) {
        $id = $data[0];
...
?>
<h5><span class="fas fa-pen"></span> Edit Kriteria</h5>
<hr>
